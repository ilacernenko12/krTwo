...
class FeedAdapter(
    ...
    override fun onBindViewHolder(holder: FeedViewHolder, position: Int) {
        ...
        holder.setItemClickListener(object : ItemClickListener {
            override fun onClick(view: View?, position: Int, isLongClick:
            Boolean) {
                if (!isLongClick) {
                    val intent = Intent(mContext,
                        OneNewsActivity::class.java)
                    intent.putExtra("TITLE", rssObject.items[position].title)
                    intent.putExtra("DATE",
                        rssObject.items[position].pubDate)
                    intent.putExtra("CONTENT",
                        rssObject.items[position].content)
                    intent.putExtra("AUTHOR",
                        rssObject.items[position].author)
                    mContext.startActivity(intent)
                }
            }
            override fun onLongClick(view: View?, position: Int, color: Int)
            {
